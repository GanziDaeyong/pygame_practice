...
def draw_score(count):
    global gamepad
    font = pygame.font.SysFont ( None, 25 ) #인자 순서는 다음과 같음 SysFont(name, size, bold=False, italic=False)
    text = font.render( str(count) + "Numung ran away!", True, WHITE) # 점수에 들어갈 내용을 작성한다. 인자 순서는 다음과 같음 render(text, antialias, color, background=None)
    gamepad.blit(text,(0,0)) # 점수를 랜더링해준다.
...
def disp_msg(text):
    ...
    large_text = pygame.font.Font("freesansbold.ttf", 50) # 폰트와 글씨 크기를 지정한다.
    text_surf = large_text.render(text, True, RED) # 텍스트를 surface 형으로. 인자순서는 위 참고
    text_rect = text_surf.get_rect() #.get_rect() 는 컨텐츠가 들어갈 칸을 만들어준다.
    text_rect.center = ((pad_width/2), (pad_height/2)) # 화면의 중간에 글씨 칸을 만든다.
    ...
    time.sleep(2) # 메세지 표시 후 2초간 텀을 둔다.
    runGame() #게임 실행 함수이다.

def crash():
    global gamepad
    
    disp_msg("Jiseung is sad..") # 적과 부딪혔을 때 나올 메세지를 입력한다.
...
def runGame():
    ...
    numung_missed = 0 # 적이 맞지 않고 지나간 횟수를 초기화시켜준다.
...
# 위에서 배경 1 뒤에 배경 2가 이어졌으므로 배경1 -> 배경2 -> 배경1 -> 배경2 로 반복되는 구조이다.
        ...
        draw_score(numung_missed) # 적을 놓친 횟수 (점수) 를 구현해준다.
        
        if numung_missed > 3 : # 만약 적을 3회 초과 놓쳤다면,
            game_over() # 게임오버된다.
        ...
        #적(누멍이) 위치설정
        numung_x -= 5 # 적(누멍이)가 왼쪽으로 5프레임씩 이동하도록
        if numung_x <= 0: # 만약 적의 x좌표가 0보다 작아지면 화면 밖을 벗어난 것이므로
            numung_missed += 1 # 놓친 적 수에 +1 한다.
        ...
        if chick[1] == None: # chick[0] 은 병아리 종류 식별자로 사용된다. (병아리 종류를 2개 이상으로 하는 경우) 병아리[1] 이 없으면
            chick_x -= 10 # 병아리가 없으면 -10의 속도로 진행한다. 단순히 시간 늦추기 용도이다.
        else:
            chick_x -= 7 # 병아리가 있으면 -7의 속도로 진행한다.
        ...
        if x + aircraft_width > numung_x: # 비행기의 폭 + x 좌표가 적의 x 좌표에 닿으면
            if (y > numung_y and y < numung_y + numung_height) or (y + aircraft_height > numung_y and y + aircraft_height < numung_y + numung_height): # 비행기의 y 좌표가 적의 y 좌표에 해당하는 범위에 닿으면
                crash() # 위에서 정의한 충돌엔딩 함수를 실행한다.
